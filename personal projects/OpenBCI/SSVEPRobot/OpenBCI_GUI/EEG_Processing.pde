//import ddf.minim.analysis.*; //for FFT
  ...
  Serial robot = new Serial(this, Serial.list()[0], 9600);
...
class EEG_Processing_User {
  ...
  public void process(float[][] data_newest_uV, //holds raw EEG data that is new since the last call
     ...
     //EEG hacker stuff (pls work)
    int tchan = 0;
    if(fftData != null) findPeakFrequency(fftData,tchan);
    
    
      if(detectedPeak[tchan].isDetected){
        String txt = "";
        if(detectedPeak[tchan].freq_Hz < processing_band_high_Hz[1-1]){
          robot.write('3');
          System.out.println("turn right");
        }
        else if(detectedPeak[tchan].freq_Hz < processing_band_high_Hz[2-1]){
          robot.write('2');
          System.out.println("turn left");
        }
        else if(detectedPeak[tchan].freq_Hz < processing_band_high_Hz[3-1]){
          robot.write('1');
