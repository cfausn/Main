...
/*               ******NOTE****** 
  You will need to change this line to the serial port assigned to your own robotic hand!!! 
*/
Serial hand =  new Serial(this, Serial.list()[32], 38400);

String oldCommand = "";
boolean isMoving = false;
boolean isClosed = false;
boolean isRocking = false;
int timeOfLastRock = 0;
...
class EEG_Processing_User {
  ...
  //**cf** variables for hand and Alpha Wave Detection
  ...
  public void process(float[][] data_newest_uV, //holds raw EEG data that is new since the last call
    ...
    if (switchesActive) {
      ...
      if ((millis() - timeOfLastTrip_R) >= 750 && (millis() - timeOfLastTrip_R) <= 1250) {
        println("rightZoneNum %i", rightZoneNum);
        if (switchTripped_L && !isLocked) {
          //**cf** removed for the hand demo for Joel's presentation, may be reimplemented later
          
          /*hand.write(Integer.toString(rightZoneNum));
          leftZoneNum = 0;
          rightZoneNum--;
          if(rightZoneNum == 0) rightZoneNum = 4;
      ...
      if ((millis() - timeOfLastTrip_L) >= 750 && (millis() - timeOfLastTrip_L) <= 1250) {
        println("leftZoneNum %i", leftZoneNum);
        if (switchTripped_R && !isLocked) {
          //**cf** removed for the hand demo for Joel's presentation, may be reimplemented later
          /*
          hand.write(Integer.toString(leftZoneNum));
          rightZoneNum = 4;
          leftZoneNum++;
          if(leftZoneNum == 5) leftZoneNum = 0;
    ...
    if (millis() - timeOfLastTrip >= 1250) {
      if (switchCounter == 1) {
      ...
      } else if (switchCounter == 2 && !isMoving) {   
        isMoving = true;
        if(oldCommand.equals("")){
          isClosed = true;
          hand.write("01234");
        }
        else if(oldCommand.equals("01234") && isClosed && !isRocking){
          isClosed = false;
          ...
          //**cf** should never get here
          isClosed = false;
          isRocking = false;
          hand.write(oldCommand); 
          hand.write("23");
        }
        else if(oldCommand.equals("23") && isClosed && !isRocking){
          isClosed = false;
          hand.write("01234"); 
        }
        else if(oldCommand.equals("23") && !isClosed && !isRocking){
        ...
        oldCommand = "01234";
        isMoving = false;
        println("Hand closed/opened");
        //lock slides
        //**cf** toggle hand movement
        //**cf** removed for the hand demo for Joel's presentation, may be reimplemented later
        /*
        if(isLocked){
          isLocked = false;
      ...
      //**cf** removed for the hand demo for Joel's presentation, may be reimplemented later
      /*
      else if (switchCounter == 3 && !isLocked) {
        //next slide
        hand.write(oldCommand);
        hand.write("23");
        oldCommand = "23";
      ...
      else if (switchCounter == 4 && !isLocked) {
        //previous slide
        println("rock on");
        hand.write("23");
        //myPresentation.slideBack();
      } else if (switchCounter == 5 && !isLocked) {
        //previous slide
        //drawPresentation = !drawPresentation;
        hand.write("5");
        println("Fist bump");
...
class EEG_Processing {
  ...
  public void process(float[][] data_newest_uV, //holds raw EEG data that is new since the last call
    ...
    if(detectedPeak[ocChan].isDetected){
      
      
      if(detectedPeak[ocChan].freq_Hz < processing_band_high_Hz[2] && detectedPeak[ocChan].freq_Hz > processing_band_low_Hz[2] && 
      (millis() - timeOfLastRock) > 5000 && !isMoving){   
        isMoving = true;
        if(oldCommand.equals("")){
          isRocking = true;
          hand.write("23");
